...
//                            $order->setState($helper->getStateByStatus($orderStatus), $orderStatus, $comment, false);
            ...
            $this->getResponse()->setBody('--- HANDLING NOTIFICATIONS PROCESS COMPLETE - MANUAL MODE ---');
        } else {
            $this->getResponse()->setBody('--- NOTIFICATIONS PROCESS IN MANUAL MODE IS DISABLED ---');
        }
    }
    ...
    public function handleAutoAction()
    {
        ...
        if($notificationHelper->isAutoMode()) {
            ...
            return $this->getResponse()->setBody($params['communication_id']);
        } else {
